//! Test safe API pipeline creation with AMD driver
...
fn main() {
    ...
    println!("Testing Safe API Pipeline Creation with AMD");
    println!("===========================================");
    
    // Create context with AMD preference
    let ctx = match ComputeContext::builder()
        .app_name("Pipeline Test")
        .prefer_icd_path("/usr/lib/x86_64-linux-gnu/libvulkan_radeon.so")
        .build()
    {
        Ok(ctx) => {
            println!("✓ ComputeContext created with AMD driver");
            ctx
        }
        Err(e) => {
